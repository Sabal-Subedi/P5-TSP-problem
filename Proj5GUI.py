...
class PointLineView(QWidget):
    ...
    def clearEdges(self, removeColors=None):
        ...
        if (
            removeColors
        ):  # allows removal of edge labels without removing node labels, for example
    ...
    def addEdge(self, startPt, endPt, label, edgeColor, labelColor=None, xoffset=0.0):
        ...
        midp = QPointF(
            (edge.x1() * 0.2 + edge.x2() * 0.8), (edge.y1() * 0.2 + edge.y2() * 0.8)
        )
...
class Proj5GUI(QMainWindow):
    ...
    def generateClicked(self):
        ...
        self.view.addPoints(
            [QPointF(c._x, c._y) for c in self._scenario.getCities()], (0, 0, 0)
        )
    ...
    def solveClicked(
        ...
        # app.processEvents()
        solve_func = (
            "self.solver." + self.ALGORITHMS[self.algDropDown.currentIndex()][1]
        )
    ...
    def checkGenInputs(self):
        ...
        if self._scenario:
            if (
                self.genParams["seed"] == seed
                and self.genParams["size"] == size
                and self.genParams["diff"] == diff
            ):
